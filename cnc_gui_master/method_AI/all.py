...
model_path = os.path.join(current_path, 'r2_model.h5')
...
def get_roi(image, ps, n):
    ...
    if n == 1:
        ...
        fixed_path = os.path.join(current_path, 'r3')
        if not os.path.exists(fixed_path):
            os.makedirs(fixed_path)
        file_path = os.path.join(fixed_path, 'roi3.png')
        cv2.imwrite(file_path, roi)
    elif n == 4:
    ...
    if n == 1:
        ...
        fixed_path = os.path.join(current_path, 'resized_r3')
        if not os.path.exists(fixed_path):
            os.makedirs(fixed_path)
        resized_file_path = os.path.join(fixed_path, 'resized_r3.jpg')
        cv2.imwrite(resized_file_path, roi)
    elif n == 4:
        fixed_path = os.path.join(current_path, 'resized_r3')
        if not os.path.exists(fixed_path):
...
def pre_proc(img, ps):
    res = image_proc(img, ps)
    cv2.imwrite('roi.png', res)
    return res
...
@app.route('/predict_r2', methods=['GET'])
def load_pic():
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.jpg', '.png', 'jpeg')):
            ...
            if img is None:
                ...
                continue
            get_roi(img, ps=ps1, n=1)
            get_roi(img, ps=ps2, n=2)
            r2 = pre_proc(img, ps=model_ps2)
...
@app.route('/predict_r3', methods=['GET'])
def load_pic():
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.jpg', '.png', 'jpeg')):
            file_path = os.path.join(input_dir, filename)
            ...
            if img is None:
                ...
                continue
            # get_roi(img, ps=ps1)
            get_roi(img, ps=ps3, n=3)
            r3 = pre_proc(img, ps=ps3)
            predicted_class, predictions = predict(r3)
            print(predicted_class)
            # update_sql_r2(int(predicted_class))
            save_predict(img, filename, predicted_class, predictions)
    return 'Processing complete', 200
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.jpg', '.png', 'jpeg')):
            ...
            if img is None:
                print(f"Can't read the file: {file_path}")
                continue
            get_roi(img, ps=ps4, n=4)
            r4 = pre_proc(img, ps=ps4)
