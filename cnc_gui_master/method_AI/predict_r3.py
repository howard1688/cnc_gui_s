...
model_path = os.path.join(current_path, 'r3_model.h5')
...
# 還未改
def update_sql_r3(flusher_level_result_value):
...
def pre_proc(img):
    roi = get_roi(img, ps3, 1)
    res = image_proc(img, ps3)
...
def save_predict(image, filename, predicted_class):
    ...
    base_dir = os.path.join(os.getcwd(), 'r3_res')
    create_dir(base_dir)
...
def load_pic():
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.jpg', '.png', 'jpeg')):
            ...
            update_sql_r3(int(predicted_class))
